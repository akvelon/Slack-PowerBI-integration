...
// ReportEngineConfig controls cmd/reportengine behavior.
...
func NewBaseConfig(p Provider) (*BaseConfig, error) {
	c := BaseConfig{
		...
		OAuthConfig: oauth.Config{
			Config: oauth2.Config{
				...
				RedirectURL:  p.Get("REDIRECT_URL", ""),
	...
	c.MaxAttempts = getInt(p, "RETRYSTRATEGY_MAXATTEMPTS", 3)
	if c.MaxAttempts < 0 {
		return nil, fmt.Errorf("the number of attempts must be greater than zero")
	}

	return &c, nil
}

// NewReportEngineConfig creates a ReportEngineConfig.
func NewReportEngineConfig(p Provider) (*ReportEngineConfig, error) {
	base, err := NewBaseConfig(p)
	if err != nil {
		return nil, err
	}

	c := ReportEngineConfig{
		BaseConfig: base,
	}
