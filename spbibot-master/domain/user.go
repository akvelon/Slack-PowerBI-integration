...
// NewSlackUserID creates a new user id.
func NewSlackUserID(workspaceID, userID string) *SlackUserID {
	id := SlackUserID{
		ID:          userID,
...
// SlackUserIDFromInteractionCallback extracts unique user id from InteractionCallback.
func SlackUserIDFromInteractionCallback(c *slack.InteractionCallback) *SlackUserID {
	return NewSlackUserID(c.User.TeamID, c.User.ID)
}

// SlackUserIDFromSlashCommand extracts unique user id from SlashCommand.
func SlackUserIDFromSlashCommand(c *slack.SlashCommand) *SlackUserID {
	return NewSlackUserID(c.TeamID, c.UserID)
}
